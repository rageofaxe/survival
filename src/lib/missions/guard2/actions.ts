import { probability, random, shuffle, sub } from '$lib/utils';
	...
	if (probability(HIRING_PROBABILITY)) {
		player.update((p) => ({
			...
			itemBasket: {
				...p.itemBasket,
				items:
					p.itemBasket.items.length < p.itemBasket.size
						? [
								...p.itemBasket.items,
								{
									name: shuffle(['!', '@', '#', '$', '%'])[0]
...
export function individualHiring(player: Writable<App.Player>) {
	const salary = 150;
	const m = get(mission);
	if (probability(HIRING_PROBABILITY) && sub(m.resources['Money'].volume as number, salary)) {
		mission.update((m) => {
